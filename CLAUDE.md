...
- **Schema changes have no migration tool.** Edit [db/models.py](backend/db/models.py). `init_db()` only creates missing tables — it won't alter existing ones — so changes to existing columns must be applied to Postgres by hand (or by wiping affected tables).
...
- **There is one database, shared everywhere.** `db/models.py` reads `DATABASE_URL` and falls back to a local SQLite file (`backend/job_agent.db`) only if it's unset. In practice `DATABASE_URL` is set in both `backend/.env` (local) and the GitHub Actions secret (cron), so local dev, the cron, and any hosted backend all read+write the same Postgres — there is no separate "local DB". A stale `backend/job_agent.db` may exist from earlier runs but isn't read while `DATABASE_URL` is set. The `backup_db()` step in `main.py` and the `backend/backups/` directory are SQLite-only and silently no-op against Postgres.
